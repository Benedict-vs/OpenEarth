...
@router.post("/timelapse/preflight", dependencies=[Depends(ensure_ee)])
def preflight(body: PreflightRequest, cache: CacheDep) -> PreflightOut:
    return svc.preflight(body, cache)
...
@router.get(
    "/timelapse/{render_id}/still/{index}",
    responses={200: {"content": {"image/png": {}}}},
)
def get_still(render_id: str, index: int, settings: SettingsDep) -> FileResponse:
    return svc.still_response(settings, render_id, index)
...
def download_movie(
    ...
    variant: Annotated[str | None, Query(pattern="^(1:1|9:16)$")] = None,
) -> FileResponse:
    return svc.download_response(engine, settings, render_id, variant)
