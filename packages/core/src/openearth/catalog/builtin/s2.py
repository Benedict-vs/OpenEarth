"""Built-in Sentinel-2 products (spectral indices, raw bands, methane proxies).

Ported from the v1 registry. Defaults changed in v2: generic products render
from L2A surface reflectance (S2_SR_HARMONIZED); the methane proxies pin L1C
TOA (S2_HARMONIZED) explicitly because the point-source retrieval literature
(Varon et al. 2021) operates on TOA reflectance.
"""
...
S2_PRODUCTS: dict[str, ProductSpec] = {
    ...
    "LAI": ProductSpec(
        ...
        bands=["B7", "B5"],
        expression=("3.618 * ((B7 / B5) - 1) - 0.118"),
        vis_min=0.0,
        vis_max=6.0,
        valid_min=0.0,
        valid_max=8.0,
    ...
    "BAI": ProductSpec(
        ...
        bands=["B4", "B8"],
        expression=("1.0 / ((0.1 - B4) * (0.1 - B4) + (0.06 - B8) * (0.06 - B8))"),
        vis_min=0.0,
        vis_max=50.0,
        valid_min=0.0,
        valid_max=100.0,
    ...
    "TURB": ProductSpec(
        ...
        name="Turbidity / TSM",
        bands=["B4", "B3"],
        expression=("1246.0 * ((B4 - B3) / (B4 + B3)) + 340.0"),
        vis_min=0.0,
        vis_max=500.0,
        valid_min=0.0,
        valid_max=500.0,
    ...
    "MBSP": ProductSpec(
        ...
        methane_only=True,
        # Methane proxies stay on L1C TOA (retrieval literature convention).
    ...
    "CH4_ANOMALY": ProductSpec(
        key="CH4_ANOMALY",
        name="Methane Enhancement (B12/B11 anomaly)",
        bands=["B12", "B11"],
        # Not expressible as per-image band math: the anomaly is target ratio
        # minus a reference-period mean ratio. The v1 registry carried a
        # vestigial expression here that silently rendered the plain B12/B11
        # ratio through the generic path — the wrong product on the map.
        expression=None,
        builder="methane_anomaly",
        vis_min=-0.08,
        vis_max=0.02,
        valid_min=-1.0,
        valid_max=1.0,
...
METHANE_S2_KEYS: list[str] = [k for k, v in S2_PRODUCTS.items() if v.methane_only]

S2_DATASET = DatasetSpec(
    id="s2",
    title="Sentinel-2 MSI (Harmonized)",
    collection_id=S2_SR_COLLECTION_ID,
