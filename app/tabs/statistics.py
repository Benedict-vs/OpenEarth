...
def _fmt(value: float, cfg) -> str:
    """Format a value with the gas display scale and unit."""
    return f"{value * cfg.display_scale:.4g} {cfg.display_unit}"
...
def render(
    ...
    if df.empty:
        st.warning("No valid observations for statistics.")
        return
    ...
    m4, m5 = st.columns(2)
    m4.metric("Min", _fmt(min_val, cfg), delta=str(min_date))
    m5.metric("Max", _fmt(max_val, cfg), delta=str(max_date))

    # ── 2. Percentile breakdown ───────────────────────────
    st.markdown("#### Percentiles")
    st.caption(
        "Percentiles show the value below which a given "
        "percentage of days fall. For example, P25 means "
        "25% of days had a concentration at or below that "
        "value. P50 equals the median."
    )
    pcts = [10, 25, 75, 90]
    ...
    hist_chart = (
        ...
        .encode(
            alt.X("value:Q", bin=alt.Bin(maxbins=40),
    ...
    rules = pd.DataFrame({
        "val": [mean_val * cfg.display_scale, median_val * cfg.display_scale],
        "label": ["Mean", "Median"],
    })
    rule_chart = (
        alt.Chart(rules)
    ...
    st.altair_chart(hist_chart + rule_chart, use_container_width=True)

    # ── 4. Monthly box plot (seasonal pattern) ────────────
    st.markdown("#### Seasonal Pattern")
    df["month"] = df["date"].dt.month
    df["month_name"] = df["date"].dt.strftime("%b")
    df["display_value"] = df["value"] * cfg.display_scale
    ...
    # ── 5. Anomaly detection (beyond 2σ) ──────────────────
    st.markdown("#### Anomalies")
    upper_bound = mean_val + 2 * std_val
    lower_bound = mean_val - 2 * std_val
    anomalies = df[(values > upper_bound) | (values < lower_bound)].copy()

    if anomalies.empty:
        st.info(
            "No anomalous days detected "
            "(all values within 2 standard deviations)."
        )
    else:
        st.caption(
            f"**{len(anomalies)}** of {n_obs} days "
            f"({len(anomalies) / n_obs:.1%}) fall outside "
            f"2\u03c3 of the mean "
            f"({_fmt(lower_bound, cfg)} \u2013 {_fmt(upper_bound, cfg)})."
        )

        plot_df = df[["date", "display_value"]].copy()
        plot_df["anomaly"] = (
        ...
        points = (
            base
            .transform_filter(alt.datum.anomaly == True)  # noqa: E712
            .mark_circle(size=50, color="#d53e4f")
        )
        band_df = pd.DataFrame({
            "lower": [lower_bound * cfg.display_scale],
            "upper": [upper_bound * cfg.display_scale],
        })
        band = (
            alt.Chart(band_df)
        ...
        st.altair_chart(
            (band + line + points).properties(height=250),
            use_container_width=True,
        )

        with st.expander("Anomalous days", expanded=False):
            show_df = anomalies[
                ["date", "value", "coverage_fraction"]
            ].copy()
            show_df["value"] = show_df["value"] * cfg.display_scale
            show_df.columns = [
                "Date",
    ...
    if date_range > pd.Timedelta(days=365):
        ...
        for col in ["mean", "median", "std", "min", "max"]:
            yearly[col] = yearly[col].apply(lambda v: _fmt(v, cfg))
        yearly.columns = [
    ...
    coverage = pd.to_numeric(
        chart_df["coverage_fraction"], errors="coerce",
    )
    ...
    q3.metric("Low-Coverage Days (<10%)", str(low_cov_days))
