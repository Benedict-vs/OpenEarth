"""Tab 3: Statistics -- summary, distribution, seasonality, anomalies."""
...
def render(
    ...
    with st.expander("Percentiles", expanded=False):
        st.caption(
    ...
    # ── 5. Monthly box plot (seasonal pattern) ────
    with st.expander(
        "Seasonal Pattern", expanded=True,
    ):
    ...
    with st.expander("Anomalies", expanded=False):
        ...
        if anomalies.empty:
            st.info(
                f"No anomalous days detected "
                f"(all values within "
                f"{sigma:.1f}\u03c3)."
            )
        else:
            st.caption(
                f"**{len(anomalies)}** of {n_obs} "
                f"days "
                f"({len(anomalies) / n_obs:.1%}) "
                f"fall outside "
                f"{sigma:.1f}\u03c3 of the mean "
                f"({_fmt(lower_bound, cfg)} \u2013 "
                f"{_fmt(upper_bound, cfg)})."
            )

            # Ensure display_value column exists
            if "display_value" not in df.columns:
                df["display_value"] = (
                    df["value"] * cfg.display_scale
                )
