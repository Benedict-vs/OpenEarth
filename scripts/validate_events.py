...
the published value. Exits non-zero if either target is outside tolerance.
...
def korpezhe() -> tuple[float, float, bool, str]:
    """Korpezhe 2018-06-19 — published 11.2 ± 5.2 t/h (GHGSat-D: 11.6 ± 8.8).

    Pass: Q ∈ [5.6, 16.8] t/h (published ±50 %) with σ overlapping.
    """
    ...
    return q, sigma, (lo <= q <= hi and overlaps), f"published 11.2 ± 5.2 t/h; pass [{lo}, {hi}]"


def hassi_messaoud() -> tuple[float, float, bool, str]:
    ...
    return mean, 0.0, (lo <= mean <= hi), detail
...
def main() -> int:
    ...
    for name, fn in (("Korpezhe", korpezhe), ("Hassi Messaoud", hassi_messaoud)):
        q, sigma, ok, detail = fn()
        all_pass = all_pass and ok
        ours = f"{q:.2f} ± {sigma:.2f}" if sigma else f"{q:.2f} (mean)"
        print(f"{name:<20}{ours:>16}   {'PASS' if ok else 'FAIL':<7}   {detail}")
    print("-" * 90)
    print("ALL TARGETS PASS" if all_pass else "SOME TARGETS FAILED")
